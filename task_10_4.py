...
class Guest(Thread):
    ...
    def run(self):
        sleep_time = randint(3, 10)  # Время пребывания гостя за столом
        sleep(sleep_time)  # Симуляция времени принятия пищи
...
class Cafe:
    ...
    def guest_arrival(self, *guests):
        for guest in guests:
            ...
            if free_table:
                ...
                self.queue.put(guest)  # Если нет свободных столов, добавляем гостя в очередь
    ...
    def discuss_guests(self):
        while not self.queue.empty() or any(table.guest is not None for table in self.tables):
            ...
            sleep(0.5)  # Задержка для обновления состояния кафе
